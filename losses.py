...
# COSINE SIMILARITY LOSS CLASSES #
...
class SimMinLoss(nn.Module):
    """
    Minimize similarity, e.g., push representations of foreground
    and background apart.
    """
    ...
    def forward(self,
        ...
        similarity = cos_sim(embedded_bg, embedded_fg)
        ...
        if self.reduction == 'mean':
            ...
            raise ValueError("Loss reduction was not specified!")


class SimMaxLoss(nn.Module):
    """
    Maximize similarity, e.g., pull representations of background
    ...
    def forward(self, embedded_background: Tensor) -> Tensor:
        """
...
def cos_sim(
    ...
    similarity = torch.matmul(
        embedded_fg, embedded_bg.T
    )

    return torch.clamp(similarity, min=0.0005, max=0.9995)
...
class InfoNCE(nn.Module):
    ...
    A query embedding is compared with one positive key and with one or more 
    negative keys. References:
    ...
    Args:
        ...
        negative_mode: Determines how the (optional) negative_keys are handled.
            ...
            If 'unpaired', then the set of negative keys are all unrelated to any 
            positive key.
    Input shape:
        query: (N, D) Tensor with query samples (e.g. embeddings of the input).
        positive_key: (N, D) Tensor with positive samples (e.g. embeddings of 
        augmented input).
        negative_keys (optional): Tensor with negative samples (e.g. embeddings of 
        other inputs)
    ...
    def __init__(self,
                 ...
                 reduction: str = 'mean',
                 negative_mode: str = 'unpaired') -> None:
        super().__init__()
...
def info_nce(query,
    ...
    if negative_keys is not None:
        ...
        # Negative keys are implicitly off-diagnoal positive keys
...
def normalize(*xs):
    return [
        None if x is None else F.normalize(x, dim=-1) for x in xs
    ]
