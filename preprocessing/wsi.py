...
if __name__ == '__main__':
    ...
    # C17-11211-5P_13 x 0.5 um_aoi17
    # C17-2710-1DQ_13 x 0.5 um_JH_5244_aoi7
    parent_dir = '/mnt/wsi-projects/pancreas-cyto'
    out_dir = '/home/asohn3/baraslab/panc_cyto'

    fl = [
        f'{parent_dir}/C17-11211-5P_13 x 0.5 um_aoi17.tif',
        f'{parent_dir}/C17-2710-1DQ_13 x 0.5 um_JH_5244_aoi7.tif'
    ]
    ...
    for file in fl:
        wsi = Slide(path=file)
        fname = wsi.filestem
        wsi.export_thumbnail(which_z=z,
                             scale_factor=scale,
                             save_as=f'{out_dir}/{fname}.png')
        arr = wsi.vips_2_nparr_z(which_z=z)
        mask = Tiler.find_roi_normal(img=arr,
                                     fname=str(fname))
        new_w = math.floor(wsi.cytowsi[z].width / scale)
        new_h = math.floor(wsi.cytowsi[z].height / scale)
        mask = cv2.resize(
            src=mask,
            dsize=(new_w, new_h),
        ...
        cv2.imwrite(
            filename=f'{out_dir}/{fname}_mask.png',
            img=mask
        )
